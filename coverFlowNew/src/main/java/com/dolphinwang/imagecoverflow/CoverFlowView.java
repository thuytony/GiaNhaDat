...
public class CoverFlowView<T extends CoverFlowAdapter> extends View {
    ...
    private void touchEnded(MotionEvent event) {
        ...
        if (mTouchMoved || (mOffset - Math.floor(mOffset)) != 0) {
            ...
            if (imageClickable() && !mLongClickTriggled) {
                final int touchedImageIndex = findTouchedImage(x, y, mTopImageIndex);
                if (touchedImageIndex != INVALID_INDEX) {
                    mClickListener.onClick(this, touchedImageIndex, mAdapter.getContentImage(touchedImageIndex));
                }
            }
        }
    ...
    public interface ImageClickListener {
        void onClick(CoverFlowView coverFlowView, int position, String contentImage);
    }
